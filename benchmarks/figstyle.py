...
def apply_style():
    ...
    plt.rcParams.update({
        ...
        # use a real U+2212 minus on negative tick labels to match body text
        # (DejaVu Sans has the glyph); was ASCII hyphen-minus
...
def panel(ax, label, title=None, dx=None, dy=None, pad=(-16, 5), xanchor=None):
    ...
    xanchor: default None keeps the original axes-anchored placement EXACTLY (the
    letter tracks the axes frame). When a float, the letter x is placed at that
    FIGURE-fraction position (via fig.transFigure) while y still comes from the axes'
    top edge and pad[1] — so row-leading letters across panels with different y-tick-
    label widths align to one common page x instead of drifting with the axes frame.
    Do NOT change the default: Fig. 6 is verified against the axes-anchored mode."""
    if xanchor is None:
        ...
        ax.annotate(label, xy=(xanchor, 1), xycoords=trans,
                    xytext=(0, pad[1]), textcoords="offset points",
                    fontsize=FS["panel"], fontweight="bold", va="baseline", ha="left")
        if title:
            ax.annotate(title, xy=(xanchor, 1), xycoords=trans,
                        xytext=(9, pad[1]), textcoords="offset points",
                        fontsize=FS["label"], fontweight="regular", color="#222",
                        va="baseline", ha="left")
...
def scalebar(ax, length_px, label, loc="lower right", color="white", pad=0.07,
    ...
    center_label (opt-in, default False keeps the edge-anchored behaviour): centre the
    label over the bar. Use only when the bar is long enough that a centred label stays
    inside the axes (e.g. a 1 mm / 500 um bar on a low-mag overview)."""
    ...
    if center_label:
        tx, tha = xr + length_px / 2, "center"
    else:
        tx, tha = (xr + length_px if right else xr), ("right" if right else "left")
    ax.text(tx, yb - 0.02 * H, label, color=color, ha=tha, va="bottom",
