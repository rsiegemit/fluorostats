...
def panel_a(ax):
    ...
    for yi, v, a in zip(y, f1, alphas):
        ax.barh(yi, v, color=BLUE, alpha=a, edgecolor="#333333", linewidth=0.4,
                height=0.68, zorder=3)

    # value labels at bar ends
    for yi, v in zip(y, f1):
        ax.text(v + 0.008, yi, f"{v:.3f}", va="center", ha="left", fontsize=5,
                color="#222222")
    ...
    # placed on the Triangle row (bottom, shortest bar 0.185) in genuinely clear space
    # to the right of that bar -> live text, no halo carving into the Yen bar above
    ax.text(0.335, 0.0,
            f"training-free fluorostats =\n{pct:.0f}% of the trained model",
            fontsize=6.0, va="center", ha="left", color=BLUE, zorder=5)
...
def main():
    ...
    fig = plt.figure(figsize=(figstyle.TW, 5.116))
    gs = fig.add_gridspec(2, 1, height_ratios=[1.0, 1.02],
                          hspace=0.327, left=0.235, right=0.965,
                          top=0.9628, bottom=0.1096)
    axes = [fig.add_subplot(gs[0]), fig.add_subplot(gs[1])]
    panel_a(axes[0])
    panel_b(axes[1])
    figstyle.panel(axes[0], "a", "DSB2018 — StarDist\u2019s own dataset", xanchor=0.0315)   # row-leader
    figstyle.panel(axes[1], "b", "unseen 3D modalities (training-free)", xanchor=0.0315)   # row-leader
