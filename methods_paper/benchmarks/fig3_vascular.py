...
axins = axd.inset_axes([0.64, 0.12, 0.32, 0.38])
axins.scatter(sps.rankdata(ve), sps.rankdata(fs), s=12, color=BLUE, edgecolor="none")
axins.set_title(f"rank ρ={sps.spearmanr(ve,fs).statistic:.2f}", fontsize=5.4)
axins.set_xticks([]); axins.set_yticks([]); axins.set_xlabel("VE", fontsize=5); axins.set_ylabel("fs", fontsize=5)
panel(axd, "d", dx=-0.14)
